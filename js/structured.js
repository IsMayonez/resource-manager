...
function sort_endpoints_by_symbol() {
  ...
  endpointSymbolCache = Object.keys(endpointSymbolCache).sort().reduce(function(obj, key) {
    obj[key] = endpointSymbolCache[key];
    return obj
  }, {});
  symbolCache = Object.keys(symbolCache).sort().reduce(function(obj, key) {
    obj[key] = symbolCache[key];
    return obj
  }, {});
...
function sort_endpoints_by_schema() {
  ...
  endpointSchemaCache = Object.keys(endpointSchemaCache).sort().reduce(function(obj, key) {
    obj[key] = endpointSchemaCache[key];
    return obj
  }, {});
  schemaCache = Object.keys(schemaCache).sort().reduce(function(obj, key) {
    obj[key] = schemaCache[key];
    return obj
  }, {});
...
function handle_properties(endpointName) {
  if ($("#properties_table li").length == 0) {
